...
```
bundle exec jekyll serve   # → http://127.0.0.1:4000, auto-regenerates on save
```

Run it in a normal interactive shell (the one rbenv is wired into). `_site/` is build output — gitignored, regenerated by GitHub Pages; don't hand-edit.

**Local preview works again as of 2026-06-20** (this reverses the earlier "it's broken" note). The fix and the setup it relies on:

- Local dev uses **plain Jekyll 4** ([Gemfile](Gemfile)), not the `github-pages` gem. Jekyll 4 gets Sass from the prebuilt `sass-embedded` (Dart Sass) binary, so there's no `sassc`/LibSass C++ compile. GitHub Pages still builds the deployed site with its own environment and ignores this Gemfile, so production is unaffected. Local Jekyll 4 vs production Jekyll 3.9 can differ in tiny ways, none that matter for this site.
- Ruby is **3.4.8 via rbenv** (`.ruby-version`), not Apple's system Ruby 2.6. rbenv was installed but never wired into the shell; `eval "$(rbenv init - zsh)"` is now in `~/.zshrc`.
- The one native gem left in the Jekyll 4 tree is **eventmachine** (pulled by `em-websocket`, used only by `serve --livereload`). It's C++. This machine's **Command Line Tools are half-broken**: the libc++ headers exist under the SDK (`$(xcrun --show-sdk-path)/usr/include/c++/v1`) but not in the toolchain dir (`/Library/Developer/CommandLineTools/usr/include/c++/v1`), so bare `clang++` can't find `<utility>` etc., and Ruby's build recorded `CXX = false`. Two workarounds are in place: rbenv's `rbconfig.rb` was patched `CXX "false" → "clang++"` (backup at `rbconfig.rb.bak`), and `bundle install` was run with `CPLUS_INCLUDE_PATH=$(xcrun --show-sdk-path)/usr/include/c++/v1` so eventmachine compiled. That compile is one-time; `jekyll serve` needs no special env afterwards.
- Proper long-term fix for the toolchain (optional): reinstall the Command Line Tools (`sudo rm -rf /Library/Developer/CommandLineTools && xcode-select --install`), after which the `rbconfig` patch and the `CPLUS_INCLUDE_PATH` dance become unnecessary (a fresh `rbenv install` would detect `clang++` on its own).
